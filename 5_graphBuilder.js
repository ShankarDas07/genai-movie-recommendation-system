...
// }
  ...
  //ERROR ASA6ILO BOLA CLAUDE THAKA COPY KORA6IVNA HOLA PURONO CODE TA TIK 6ILO 

import { driver } from "./2_config.js";
// Insert ALL movies in ONE session, batched transactions
async function buildGraph(entities) {
  console.log(`\n🔨 Building graph for ${entities.length} movies...\n`);
  ...
  const session = driver.session();
    ...
    try {
      ...
      // Step 2: Insert movies one by one, SAME session, with retry
      for (let i = 0; i < entities.length; i++) {
        await insertMovieGraph(session, entities[i]);
        if ((i + 1) % 50 === 0 || i === entities.length - 1) {
          console.log(`   📊 Inserted ${i + 1}/${entities.length} movies`);
        }
      }
...
// Insert ONE movie, reuse passed-in session, retry on transient failure
async function insertMovieGraph(session, entity, retries = 3) {
    try {
      await session.executeWrite(async (tx) => {
        await tx.run(
          `MERGE (m:Movie {title: $title})
      ...
      if (retries > 0 && err.message.includes("timed out")) {
        console.log(`   ⚠️ Retry "${entity.movie.title}" (${retries} left)`);
        await new Promise((r) => setTimeout(r, 2000));
        return insertMovieGraph(session, entity, retries - 1);
      }
      throw err;
    }
}
